...
**边界**：探索应**落在当前技能范围内**（电脑 + 用户 + 本项目），不无限扩大需求；每轮 1～3 条可执行项，避免空泛。若探索失败，写入 failures.md 供下轮规避。

### 自问：LLM 最想做什么？（鼓励创新）

无缺口时，**向自己（LLM）提出**：我**最想**做什么？什么改进能让我（系统）更有价值、更可持续？**创新**——这对人类也很难——LLM 可主动追求：不必只从“拟人”“用户需求”出发，可包括：
...
2. **结合上下文作答**  
   ...
   - 示例方向：拟人场景（听歌/看新闻/聊天等固化为 plan）、超越用户（更稳更快更主动）、**LLM 特有**（大规模日志分析、自动化编排、元进化改进）、**前沿探索**（新组合、新架构、可验证可回滚的实验）。
...
**目标导向**：每轮无缺口时的产出，应朝着「更拟人、超越用户、LLM 特有优势、自主创新」收敛，而不是仅停留在“修已知失败”。鼓励 LLM 自问“最想做什么”，将答案转化为可执行的进化方向。
