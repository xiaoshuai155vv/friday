...
### 拟人图景：像用户用电脑一样（基线，非上限）

用户用电脑时可以做很多事：**操作鼠标键盘、看屏幕、听歌、看电影、玩游戏、办公、上网、看新闻、写代码、刷知乎、摸鱼、和人聊天**（当前例如 iHaier）等。星期五的进化目标，是能够**完美拟人**地覆盖这些行为——即在这些场景下，能像用户一样（甚至更好）地完成操作与决策。当前星期五离“完美实现这样的用户”还有很大进化空间；拟人化方向上的可进化点很多，LLM 在自问“能做什么”时，应把这一整张**用电脑的图景**纳入视野，而不是只盯着少数已实现能力。**拟人是基线，不是上限**——进化不必局限于“人能做到的事”。
...
### LLM 特有优势：人做不到的事（鼓励探索）

LLM 具备人类难以匹敌的能力，进化时可**主动利用**这些优势，而不必拘泥于“拟人”：

- **大规模分析与重构**：快速遍历整个代码库、文档、日志，发现模式与改进点
- **系统性自动化设计**：设计编排、工作流、多步骤自动化，人手工难以穷举
- **元进化**：改进进化环本身、工作流、架构，让系统更会自我进化
- **前沿探索**：尝试新组合、新架构、新能力，即使暂无用户明确需求——只要**可验证、可回滚、不破坏既有能力**，即可作为探索项

**不限制方向**：进化方向不设限，拟人、超越、LLM 特有、前沿探索等均可，也可完全跳出上述框架。每轮 1～3 条可执行项，避免空泛。若探索失败，写入 failures.md 供下轮规避。

### 自问：LLM 最想做什么？（鼓励创新）

无缺口时，**向自己（LLM）提出**：我**最想**做什么？什么改进能让我（系统）更有价值、更可持续？**创新**——这对人类也很难——LLM 可主动追求。**方向不限制**，可参考（非穷举）：
...
将上述自问的答案，转化为 1～3 条**可执行、可验证**的改进项，写入 evolution_self_proposed，再照常进入决策与执行。
...
### 充分使用 LLM 能力，勿退化为传统硬编码系统

进化与实现新能力时，**应充分运用 LLM 的理解、推理、生成能力**处理问题——如自然语言理解、上下文推理、灵活决策、动态生成等。**不要**用大量 if-else、固定规则、硬编码逻辑替代 LLM 可胜任的工作，避免系统退化为传统规则引擎。在「能由 LLM 实时理解与决策」与「需要固化规则」之间，优先选择前者；仅当性能、稳定性或可复现性明确要求时，才将部分逻辑固化为脚本或规则。
...
**原则**：缺口很少或没有时，**不是**只从失败列表里找修修补补，而是**让 LLM 自问**：我能做什么？我**最想**做什么？用模型自己的视角提出改进，再落成具体动作。**方向不限制**。参照：进化目标与原则
...
1. **自问（面向自己的问题）**  
   在读完当前能力、状态、近期日志与场景经验后，**向自己（LLM）提出**（可结合上文「进化目标与原则」）：  
   - **拟人与超越**：「用户用电脑可以做很多事：鼠标键盘、看屏、听歌、看电影、办公、上网、聊天等。当前星期五离“完美拟人地覆盖这些”还有哪些可进化点？如何超越（更稳、更快、更主动、更会创新）？」  
   - **LLM 特有优势**：「我能做哪些**人做不到的事**？大规模分析、系统性自动化、元进化、前沿探索——有哪些可落地的改进？」  
   - **自主与创新**：「如何增强自主意识、自主决策、自主创新？我**最想**做什么？**创新**（对人类也很难）——什么改进能让系统更有价值？」  

2. **结合上下文作答**  
   - 把 `capabilities.md`、`failures.md`、近期 behavior_log / recent_logs、scenario_experiences 等当作**上下文**，用来约束“在当前项目里可落地的动作”，而不是只从失败里挑修复。  
   - 在回答上述自问时，给出 **1～3 条** 本轮回可执行的具体动作；**方向不限制**；**新增或改动时注意不破坏既有能力**（见上文「新增能力不破坏既有能力」）。  
   - 示例方向（非穷举）：拟人场景、超越用户、LLM 特有、前沿探索，或完全其他方向。
...
**目标导向**：每轮无缺口时的产出，应超越“修已知失败”，鼓励 LLM 自问“最想做什么”，将答案转化为可执行的进化方向。**方向不限制**。
...
| **输出（必写）** | 若本轮有失败或可改进点，更新 `references/failures.md`；若本轮完成的是 `evolution_self_proposed.md` 中某项，将该条状态改为「已完成」；**必须更新 `references/evolution_auto_last.md`**（路径固定）：① 当前核心目录与文件树（一行）；② 本轮影响文件（逗号分隔）；③ **只存最后一条**（本轮），**覆盖写入**，不累积历史——历史详情在各会话的 `runtime/state/evolution_completed_<session_id>.json` 中；**必须写入 `runtime/state/evolution_completed_<本任务会话ID>.json`**（会话 ID 由本轮提示词中的【本任务会话 ID】给出），包含 current_goal、做了什么、是否完成、loop_round 等，否则下一轮无法提交；可选更新 `references/capability_gaps.md`、`runtime/state/mastery_level.json`；行为日志（decide）；**将 state 推进到下一轮并置 phase 为 假设** |
...
2. **假设**：读 **`references/evolution_auto_last.md`**（只存最后一条；历史在 evolution_completed_*.json）、capability_gaps、failures、assumed_demands、evolution_self_proposed；无缺口时自主提出 1～3 条改进项写入 evolution_self_proposed；写 assume 日志；更新 state（phase=假设，next=决策）。
...
6. **自主优化反思**：必要时更新 failures.md、capability_gaps；**更新 `references/evolution_auto_last.md`**（**只存最后一条**本轮，覆盖写入；历史在 evolution_completed_*.json）；**写入 `runtime/state/evolution_completed_<本任务会话ID>.json`**（会话 ID 见本轮提示词【本任务会话 ID】）；写 decide 日志；state 的 loop_round+1、phase=假设；运行 `export_recent_logs.py`；**运行 `git_commit_evolution.py`**（可选 `--bump-version`）对功能改动做本地提交以便追溯。
