...
| 剪贴板 | 已覆盖：clipboard_tool get/set 文本 | 图片剪贴板可扩展 CF_DIB |
| 电源/睡眠 | 已覆盖：power_tool prevent_sleep 阻止休眠与关屏 | 休眠/关机可 powercfg 或 SetSuspendState |
...
| 窗口激活/前后台 | 未覆盖 | ctypes FindWindow/SetForegroundWindow 或 组合键 Alt+Tab |
| 进程列表/结束 | 已覆盖：process_tool.py list / kill；tasklist / taskkill | 按窗口标题查 PID 可扩展 |
| 关机/休眠/重启 | 未覆盖 | powercfg / SetSuspendState / ExitWindowsEx |
