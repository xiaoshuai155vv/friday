...
- **通用多模态输入（无需在场景 JSON 里写）**：vision_proxy **自动注入**当前屏幕逻辑分辨率与图片像素尺寸到每次请求的提示中，要求模型在此尺寸下返回坐标等。**场景/计划 JSON 中无需再写分辨率、图片尺寸说明**。
...
- **坐标类 vision 加 coords**：当 vision 步骤需要返回点击坐标时，加 `"coords": true`，run_plan 会调用 **vision_coords.py**（内部多轮 vision_proxy 取中位数），提高坐标稳定性；非坐标类（如读消息内容）用 vision_proxy。

## Vision 坐标校准（维护偏移数据集）

多模态返回的坐标常存在系统性偏移（如整屏少 x、DPI/分辨率差异）。可通过**屏幕校准**得到当前环境下的偏移量并持久化，run_plan / click_from_vision_or_key 会自动加上该偏移再点击。
