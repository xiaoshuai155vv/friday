...
| 执行计划（截图/vision/点击/输入等） | `python scripts/run_plan.py assets/plans/xxx.json`；步骤类型：screenshot / vision / vision_coords / click / type / key / paste / scroll / wait / run；默认打印多模态输入输出（`--no-verbose` 可关闭）、自动启动 Qt 悬浮球显示进度（`--no-floating` 可跳过） |
