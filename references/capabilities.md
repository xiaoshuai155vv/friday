...
| **获取点击坐标**（多轮取中位数） | `python scripts/vision_coords.py <图路径> "问题"` 或 run_plan 中 `{"do": "vision_coords", ...}`；加 `"normalized": true` 用归一化坐标（0-1 转像素）；设 `FRIDAY_VISION_VERBOSE=1` 打印提示词与模型输出 |
