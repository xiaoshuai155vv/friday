...
def main():
    ...
    # round 662: 智能全场景进化环元进化系统自驱动持续优化引擎 - 在 round 661 完成的元进化架构自省与认知迭代引擎基础上，构建让系统能够基于架构自省的分析结果自动驱动并执行持续优化，形成「自省→决策→执行→验证→再自省」的完整闭环
    # 实现「自省→决策→执行→验证→再自省」的完整自驱动持续优化闭环，形成持续优化的进化飞轮
    elif "自驱动" in intent or "持续优化" in intent or "self driven" in intent.lower() or "continual" in intent.lower() or "optimization loop" in intent.lower() or "优化闭环" in intent or "自动优化" in intent or "自驱动优化" in intent:
        print(f"[元进化系统自驱动持续优化引擎 v1.0.0] 正在处理...", file=sys.stderr)
        script_path = os.path.join(SCRIPTS, "evolution_meta_self_driven_continual_optimizer_engine.py")
        cmd_args = sys.argv[1:] if len(sys.argv) > 1 else ["--run-cycle"]
        filter_words = ["自驱动", "持续优化", "self driven", "continual", "optimization loop", "优化闭环", "自动优化", "自驱动优化"]
        filtered_args = [arg for arg in cmd_args if not any(w in arg.lower() for w in filter_words)]
        if not filtered_args:
            filtered_args = ["--run-cycle"]
        result = subprocess.run(
    ...
    # round 663: 智能全场景进化环元进化优化策略自评估与自适应调整引擎 - 在 round 662 完成的系统自驱动持续优化引擎基础上，构建让系统能够评估自身优化策略有效性的能力，形成「优化→评估→自适应调整」的完整递归闭环
    # 实现元元优化（meta-optimization）能力，让系统从「能优化」升级到「学会如何优化」
    elif "元优化" in intent or "策略评估" in intent or "策略自评估" in intent or "meta optimization" in intent.lower() or "strategy evaluation" in intent.lower() or "self evaluation" in intent.lower() or "优化评估" in intent or "学会如何优化" in intent or "自适应调整" in intent:
        print(f"[元进化优化策略自评估与自适应调整引擎 v1.0.0] 正在处理...", file=sys.stderr)
        script_path = os.path.join(SCRIPTS, "evolution_meta_optimization_strategy_self_evaluation_engine.py")
        cmd_args = sys.argv[1:] if len(sys.argv) > 1 else ["--run-cycle"]
        filter_words = ["元优化", "策略评估", "策略自评估", "meta optimization", "strategy evaluation", "self evaluation", "优化评估", "学会如何优化", "自适应调整"]
        filtered_args = [arg for arg in cmd_args if not any(w in arg.lower() for w in filter_words)]
        if not filtered_args:
            filtered_args = ["--run-cycle"]
        result = subprocess.run(
    ...
    # round 664: 智能全场景进化环元元元进化与递归优化有效性评估引擎 - 在 round 663 完成的元元优化能力基础上，构建让系统能够评估元元优化本身有效性的能力，形成「元优化→元元优化→元元元优化」的递归升级闭环
    # 实现元元元（meta-meta-meta）进化能力，让系统从「学会如何优化」升级到「学会如何学会优化」
    elif "元元元进化" in intent or "元元元" in intent or "meta-meta-meta" in intent.lower() or "递归优化评估" in intent or "优化递归" in intent or "元元元优化" in intent or "学会如何学会优化" in intent or "meta meta meta" in intent.lower() or "recursive evaluation" in intent.lower():
        print(f"[元元元进化与递归优化有效性评估引擎 v1.0.0] 正在处理...", file=sys.stderr)
        script_path = os.path.join(SCRIPTS, "evolution_meta_meta_meta_evolution_recursive_evaluation_engine.py")
        cmd_args = sys.argv[1:] if len(sys.argv) > 1 else ["--run-cycle"]
        filter_words = ["元元元进化", "元元元", "meta-meta-meta", "递归优化评估", "优化递归", "元元元优化", "学会如何学会优化", "meta meta meta", "recursive evaluation"]
        filtered_args = [arg for arg in cmd_args if not any(w in arg.lower() for w in filter_words)]
        if not filtered_args:
            filtered_args = ["--run-cycle"]
        result = subprocess.run(
    ...
    # round 667: 智能全场景进化环元进化决策质量持续优化与自驱动演进引擎 - 在 round 666 完成的决策质量预测与预防性优化引擎基础上，构建让系统能够自动评估预测准确性、根据偏差调整模型参数、实现持续自我优化的能力
    # 形成从预测→验证→优化的完整自驱动闭环
    elif "决策质量持续优化" in intent or "自驱动演进" in intent or "决策质量优化" in intent or "decision quality continuous" in intent.lower() or "continuous optimization" in intent.lower() or "self-driven evolution" in intent.lower() or "自驱动优化" in intent or "模型参数调整" in intent or "预测准确性评估" in intent or "决策质量演进" in intent:
        print(f"[元进化决策质量持续优化与自驱动演进引擎 v1.0.0] 正在处理...", file=sys.stderr)
        script_path = os.path.join(SCRIPTS, "evolution_meta_decision_quality_continuous_optimizer_engine.py")
        cmd_args = sys.argv[1:] if len(sys.argv) > 1 else ["--run-cycle"]
        filter_words = ["决策质量持续优化", "自驱动演进", "决策质量优化", "decision quality continuous", "continuous optimization", "self-driven evolution", "自驱动优化", "模型参数调整", "预测准确性评估", "决策质量演进"]
        filtered_args = [arg for arg in cmd_args if not any(w in arg.lower() for w in [fw.lower() for fw in filter_words])]
        if not filtered_args:
            filtered_args = ["--run-cycle"]
        result = subprocess.run(
