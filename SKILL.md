...
├── requirements.txt
├── VERSION
├── runtime/            # 运行时生成（logs、state、screenshots），.gitignore
├── python/             # 便携环境（可提交）
└── .gitignore
```

**runtime/** 为运行时生成，不纳入技能包。**python/** 为便携环境，可提交。

**scripts 必含**：`run_with_env.py`（统一入口）、`do.py`（意图分发）、`launch_browser.py`、`run_plan.py` 等。若 `run_with_env.py` 不可用，可直接 `python scripts/launch_browser.py [url]` 或 `do.py 打开浏览器 [url]`。
...
| **自主决策** | 按进化环（假设 → 规划 → 执行 → 校验 → 反思 → 决策）自主推进；读取 `runtime/state/current_mission.json`、`capability_gaps`、`failures`、场景经验等，决定下一步目标与动作；不依赖用户逐步指令，可多轮闭环直至任务达成或能力补齐。 |
...
| **日志与溯源** | **记录所有行为**，便于溯源；`scripts/behavior_log.py` 写 `runtime/logs/`，详见 `references/logging.md`。 |
| **UI** | 类似贾维斯/星期五的**科幻主体**，不断自我完善；思考能力由当前 LLM 决定，不在 UI 上显示年龄或智商阶段。 |
| **防迷失** | **始终知道我们当前要干什么**；状态见 `runtime/state/current_mission.json`，每次行动前后读写，避免多轮循环中迷失。 |
...
- **状态位置**：`runtime/state/current_mission.json`（由 `scripts/state_tracker.py` 维护）。
...
- **校准偏移**：vision 返回的坐标常有系统性偏差，需通过 `vision_calibrate.py calibrate` 得到 offset 并写入 `runtime/state/vision_calibration.json`；run_plan / click_from_vision_or_key 会自动加上该偏移再点击。换分辨率后需重跑校准。
- **provider 选择**：可用 `vision_calibrate.py benchmark` 对比各 provider 的偏差稳定性（std 越小越稳）；benchmark 结果通常推荐 glm。

## 行为日志与溯源

- **原则**：记录所有行为，便于溯源。
- **实现**：`scripts/behavior_log.py` 写入 `runtime/logs/` 目录，每条含时间、动作类型、简要描述、关联任务/使命。
...
| **保底场景** | **打开浏览器 + 鼠标 + 键盘 + 截图 + 多模态** 的通用流程：打开浏览器（或指定 URL）→ **等待加载后激活并最大化浏览器窗口** → 截图 → 多模态看图 → 根据画面点击/输入/按键。**打开/激活任何应用后必须先最大化**，再做截图/vision 等后续操作。详见下方「保底方案：Windows 界面引导」。 |
...
**前提**：Windows 上**所有已安装应用**均可从**开始菜单**或**任务栏**搜到/看到；**不要**去文件系统搜 exe、Program Files 等。

在 **Win 电脑**上走保底「截图 → 多模态看图 → 点击/输入」时，可让多模态模型结合以下界面常识做决策，减少漏点、误点：

| 区域 | 说明与可操作方式 |
|------|------------------|
| **开始菜单** | 按 **Win 键**打开开始菜单，输入应用名即可**搜索并启动**；或 `do 打开运行`（Win+R）后输入应用名回车。**打开任意应用**优先用此方式，勿搜文件系统。 |
...
**核心原则**：**场景优先，通用能力次之**。鼠标、键盘、截图、多模态坐标、多模态识别 等通用能力**仅在没有场景匹配时**才使用；**禁止**一上来就截图+vision 逐步操作。

响应用户请求时，**必须先**按以下顺序判断，**不得跳过**：
...
| **2** | 用户需求**不匹配**任何场景 | **才**走保底：打开浏览器/应用 → **激活并最大化** → 截图 → 多模态看图 → 点击/输入。 |
| **3（最低）** | 存在非常明确的单项能力（如「截图」→screenshot_tool） | 直接使用该能力。 |

**禁止**：在未确认「无场景匹配」前，**不要**使用 截图、vision、vision_coords、mouse_tool、keyboard_tool 等通用能力；场景 steps 中的每一步（如「先获取应用列表」）**必须执行**，不得省略。
...
| 打开浏览器、打开某网址 | `python scripts/launch_browser.py [url]` 或 `do.py 打开浏览器 [url]`；**打开后须激活并最大化浏览器窗口**，再做截图/vision |
...
- `scripts/state_tracker.py` — 读写 `runtime/state/current_mission.json`，维护「当前要干什么」。
...
- `scripts/behavior_log.py` — 写行为日志到 `runtime/logs/`。
- `scripts/load_private_knowledge.py` — 按需加载私域（`get domains` / `get user_assumptions`）。
- `scripts/screen_size_tool.py`、`scripts/mouse_tool.py`（含 right_click/middle_click/drag/scroll；**scroll**：正数=向上滚/看下方更多，负数=向下滚/看上方更多，120≈1格）、`scripts/keyboard_tool.py`、`scripts/screenshot_tool.py`、`scripts/clipboard_tool.py`、`scripts/timer_tool.py`、`scripts/file_tool.py`、`scripts/time_tool.py`、`scripts/process_tool.py`（进程 list/kill）— 屏幕/鼠标/键盘/截图/剪贴板/定时/文件/时间/进程（Windows）。
...
- `scripts/window_tool.py` — 窗口激活 `activate "标题"`；**激活后须 maximize 再截图**；`maximize "标题"` / `maximize_process <进程名>`；`do.py 窗口激活/窗口PID/结束窗口`。
...
- `scripts/launch_browser.py` — 用默认浏览器打开 URL；**打开后须激活并最大化浏览器窗口**，再做截图+vision+run_plan。
- `scripts/launch_notepad.py`、`scripts/launch_explorer.py` — 打开记事本、文件管理器（可带路径）。
- `scripts/selfie.py` — 自拍：直接启动 camera_qt → 等约 4s 画面就绪 → 截屏保存 runtime/screenshots/selfie_*.bmp。用 `run_with_env` 或系统 `python` 执行时，camera_qt 会使用调用者 Python（避免便携环境检测不到摄像头）。
